...
def test_langgraph_a2a_node(monkeypatch):
    ...
    )  # base_url unused after monkeypatch
    ...
    if hasattr(node, "client") and hasattr(node.client, "send"):
        monkeypatch.setattr(
            node.client,
            "send",
            lambda text, use_jsonrpc=False: f"Hello, you said: {text}",
        )
    else:
        ...
        if hasattr(node, "__call__"):
            monkeypatch.setattr(
                ...
                lambda state: {
                    **state,
                    "messages": state["messages"] + [HumanMessage(content="Hello")],
                },
            )
            ...
            pytest.skip(
                "A2AAgentNode does not expose a patchable client or callable surface"
            )
